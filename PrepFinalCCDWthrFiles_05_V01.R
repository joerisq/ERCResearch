...
percte <- 'med'
scnio <- 'rcp85'
runspec <- paste0(CCDFlag,'_',trgtyr,percte,scnio)
chngfacts <- paste0('change_factors_',trgtyr,'_Q_',percte)
change_factors_tas <- readRDS(paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/ClimateConditioningInput/Climate_Conditioning_Change_Factors_tas.rds'))
change_factors_pr <- readRDS(paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/ClimateConditioningInput/Climate_Conditioning_Change_Factors_pr_v2.rds'))
change_factors_rh <- readRDS(paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/ClimateConditioningInput/Climate_Conditioning_Change_Factors_rh.rds'))
...
for (i in 1:length(wthrdatallpyrs.DT)) { 
  ...
  fileout <- paste0('AvgdStatDat_',runspec,'_',PYROMEIN,'.fwx')  
  ...
  if(length(file.exists(softwareflesinrnmd)) < 2 ){ 
    ...
    softwareflesrnm <- gsub('000',PYROMEIN, softwareflesin)
    for(file in softwareflesin) {
      file.copy(softwareflesin, filedir)
    }
    file.rename(softwareflesin, softwareflesrnm)
  }
