##################################################################################################################
## Project: Extracting data from RAWS and NARR data files  
##
## Script purpose: Read .fw13 and .fwx formatted RAWS data and extract all variables, averaging all stations for 
##                 each by pyrome 
##
...
##################################################################################################################
## Section: Read Station files defining closest 5 stations every Pyrome centroid
##          Augment data for pyromes 17 and 56, using closest 10 stations
##################################################################################################################
...
# Remove data for pyromes 17 and 56
STATgrd.DT <- STATgrd.DT[PYROME != '017' & PYROME != '056']

# Read station point list for pyromes 17 and 56. Top 10 stations required, as first 5 insufficient 
stationfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/Top10CloseStationsAllPyromes_2019_P017.P056.rds')
STATgrd.DT.p17.p56 <- readRDS(stationfile) 

# Append data for pyrome s17 and 56 with all pyrome data
filesappend <- list(STATgrd.DT, STATgrd.DT.p17.p56) 
STATgrd.DT.out <- rbindlist(filesappend, use.names=TRUE, fill=TRUE)  

# Save resultant file
stationlistfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top5CloseStationsAllPyromes_2019CW017.056.Suplmnt.rds')
saveRDS(STATgrd.DT.out, stationlistfile)
...
stationfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top5CloseStationsAllPyromes_2019CW017.056.Suplmnt.rds')
...
stationdata.DT.all.pyrs <- stationdata.DT.all.pyrs[ MEASTYP == 2]
...
# Write concatenated data.table of all pyromes station data combined
cmbndstdatfile <- paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/AllPyromesOrigStatDat_DDUPED.rds')
saveRDS(stationdata.DT, cmbndstdatfile)

##################################################################################################################
## Section: Create IDW distance weights and elevation corrections for temperature and relative humidity
##################################################################################################################

# Read station list for all pyromes 
stationattrbfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top5CloseStationsAllPyromes_2019CW017.056.Suplmnt_CWElev.rds')
StatAtribs.DT <- readRDS(stationattrbfile)
#StatAtribs.DT <- StatAtribs.DT[NESDIS != '________']
#StatAtribs.DT <- StatAtribs.DT[STATID != 149901]
#StatAtribs.DT[PYROME == '088']
...
# Convert to feet
elevdat.CORR <- elevdat.CORR * 3.2808399
plot(elevdat.CORR)
...
coords <- leg_centers.cntds[ , c('LONGITUDE', 'LATITUDE')] # coordinates
data   <- leg_centers.cntds[ , 1:3] # data
crs    <- CRS('+init=epsg:4326') # proj4string of coords

# make the spatial points data frame object
pyrms.spdf <- SpatialPointsDataFrame(coords = coords,
                                      data = data, 
                                      proj4string = crs)

# Project data points to DEM projection
pyrms.spdf.trnsfrmd <- spTransform(pyrms.spdf,DEMProj)

# Extract elevation at pyrome centroids
pyrms.spdf.trnsfrmd.elv <- extract(elevdat.CORR, pyrms.spdf.trnsfrmd)
pyrms.spdf.trnsfrmd.elv[ is.na(pyrms.spdf.trnsfrmd.elv) ] <- 0

# Apply based on average Elevation of locations - temperature lapse rate per thousand feet increase in elevation -3.6 F or -2.0 C 
StatAtribs.DT[,MEANELV := mean(ELEVDAT), by = PYROME]
StatAtribs.DT[,ELEVDIF := (MEANELV - ELEVDAT)/1000.0]
StatAtribs.DT[,TEMPCOR := ELEVDIF * -2]

# Create IDW weights 
# https://www.geo.fu-berlin.de/en/v/soga/Geodata-analysis/geostatistics/Inverse-Distance-Weighting/index.html
beta = 2
StatAtribs.DT[,IDW := round((1.0/(dist^beta))*10000,2)] 
StatAtribs.DT[, IDWSUM := sum(IDW), PYROME][]

# Test plot IDW/Temp correctiosn - Create data frame of locations - pyrome centroid first 
StatAtribs.DT.pyr <- StatAtribs.DT[PYROME == '034']
Pcnt <- leg_centers.cntds[ 34, c('LONGITUDE', 'LATITUDE')]
stcnts <- StatAtribs.DT.pyr[ , c('LONGITUDE', 'LATITUDE')]  
p <- as.data.frame(rbind(Pcnt, stcnts))
p0 <- p[1,] 
p1 <- p[2:nrow(p),] 
n = nrow(p) - 1

# Crude results plot
ggplot(p1, aes(x = LONGITUDE, y = LATITUDE, label = rownames(p1))) + 
  geom_point() + 
  geom_point(data = p0, aes(x = LONGITUDE, y = LATITUDE, 
...
# Chracterize pyrome ID
stationdata.DT[, PYROME := sprintf("%03d", as.integer(PYROME))] 

# Initialze combined output
DT_cmb.mstr <- data.table(NULL)
...
for (ipyr in pyromes) { 
  ...
  if (!is.na(idx_one_in_two)) { 
    ...
    # Get IDW parameters for this pyrome
    IDWPrmtrs <- StatAtribs.DT[PYROME == PYROMEIN]
    IDWPrmtrs[ ,`:=`(PYROME = NULL)]
    
    # Extract data variables 
    seltvar <- c('STATINDEX','STATID','TEMPCOR','IDW','IDWSUM')
    IDWPrmtrs <- IDWPrmtrs[, ..seltvar]

    # Get data for this pyrome
    stationdata.DT.pyr <- stationdata.DT[PYROME == PYROMEIN]
    setcolorder(stationdata.DT.pyr, 'DATE')

    # Merge IDW parameters with station data
    setkey(IDWPrmtrs,STATID)
    setkey(stationdata.DT.pyr,STATNUM)
    stationdata.DT.pyr <- merge(stationdata.DT.pyr,IDWPrmtrs, by.x='STATNUM', by.y='STATID', all = TRUE)
    
    # Calculate Dew point - Output dew point in Fahrenheit
    stationdata.DT.pyr <- stationdata.DT.pyr[,DEWPOINT := DewPnt(ATMOIST,celsius(TEMP))]
    stationdata.DT.pyr <- stationdata.DT.pyr[,DEWPOINT := fahrenheit(DEWPOINT)]    

    # Correct Temperature values
    stationdata.DT.pyr[,TEMP := round(TEMP + TEMPCOR,2)]
    stationdata.DT.pyr[,MaxT := round(MaxT + TEMPCOR,2)]
    stationdata.DT.pyr[,MinT := round(MinT + TEMPCOR,2)]
    
    # Correct RH values absed on new corrected Temperatures and estimated dew point
    stationdata.DT.pyr <- stationdata.DT.pyr[,ATMOIST := round(RelHumdty(celsius(DEWPOINT), celsius(TEMP)),0)]
    stationdata.DT.pyr <- stationdata.DT.pyr[,ATMOIST := ATMOIST] [ATMOIST > 100.0, ATMOIST := 100]
    stationdata.DT.pyr <- stationdata.DT.pyr[,MxRH := MxRH] [ATMOIST > MxRH, MxRH := ATMOIST]    
    stationdata.DT.pyr <- stationdata.DT.pyr[,MnRH := MnRH] [ATMOIST < MnRH, MnRH := ATMOIST]    
    stationdata.DT.pyr[ ,`:=`(TEMPCOR = NULL, DEWPOINT = NULL, STATINDEX = NULL)]
    sapply(stationdata.DT.pyr,class)
    namesin <- names(stationdata.DT.pyr)
    names2remove <- c('DATE','PYROME')
    namesin <- namesin[!(namesin %in% names2remove)]
    namesin <- c('DATE',namesin,'PYROME')
    setcolorder(stationdata.DT.pyr,namesin)

    # Split pyrome level dt by weather variables
    whethrvar.pyr.lst <- list()
    DT.out <- list()
    seltvar <- c('DATE','STATNUM','IDW','IDWSUM','PYROME')
    cols2procs <- c('RnDr','RAIN','TEMP','ATMOIST','MaxT','MinT','MxRH','MnRH')
    for (ivr in 1:length(cols2procs)) {
      seltvar <- c(seltvar,cols2procs[ivr])      
      DT.out[ivr] <- paste0('data_cont.',cols2procs[ivr])
      whethrvar.pyr.lst[[ivr]] <- stationdata.DT.pyr[, ..seltvar]
      seltvar <- seltvar[!(seltvar %in% cols2procs[ivr])]
    }
    names(whethrvar.pyr.lst) <- (DT.out)
    ...
    DT_cmb <- cont_date_time

    # Process each of the weather variables
    lstnmes <- names(whethrvar.pyr.lst)
    for (idt in 1:length(whethrvar.pyr.lst)) { 
      data_cont <- cont_date_time
      data_cont <- merge(data_cont, whethrvar.pyr.lst[[lstnmes[[idt]]]], by = 'DATE', all=TRUE)
      data_cont[, IDWSUM := sum(IDW), by = DATE]
      data_cont[, IDW2APLY := IDW/IDWSUM, by = DATE]
      varibl <- names(data_cont)[6]
      data_cont[, eval(varibl) := round(IDW2APLY * get(varibl),2), by = DATE]
      data_cont <- data_cont[,lapply(.SD,sum),by=DATE,.SDcols=varibl] 

      # Select weather variable
      pattern <- c('RnDr','RAIN','TEMP','ATMOIST','MaxT','MinT','MxRH','MnRH')
      tifsperrds <- as.matrix(sapply(pattern, function(x) grepl(x, colnames(data_cont))))
      rindxs <- which(apply(tifsperrds, 1, function(x) any(x == c(TRUE))))
      cindxs <- t(t(apply(tifsperrds, 1, function(x) paste(names(which(x)), collapse=","))))
      varibl <- cindxs[rindxs]
      
      if (varibl == 'RAIN') { 
        RAINExtm <- 10.0
        data_cont <- data_cont[, RAIN := RAIN ][RAIN > RAINExtm, RAIN := as.numeric(RAINExtm)]  
      } else if (varibl == 'TEMP') {
          mean.TEMP <- data_cont[ , lapply(.SD, mean, na.rm=TRUE), .SDcols = !'DATE']
          sd.TEMP <- data_cont[ , lapply(.SD, sd, na.rm=TRUE), .SDcols = !'DATE'] 
          TEMPExtm <- as.integer(unlist(mean.TEMP[[1]] + 3.0 * sd.TEMP[[1]]))
          data_cont <- data_cont[, TEMP := TEMP ][TEMP > TEMPExtm, TEMP := as.integer(TEMPExtm)]
      } else if (varibl == 'ATMOIST') {
          data_cont <- data_cont[, ATMOIST := ATMOIST ][ATMOIST > 100, ATMOIST := as.integer(100)]
      } else if (varibl == 'MinT') {
          mean.TEMP <- data_cont[ , lapply(.SD, mean, na.rm=TRUE), .SDcols = !'DATE']
          sd.TEMP <- data_cont[ , lapply(.SD, sd, na.rm=TRUE), .SDcols = !'DATE'] 
          TEMPExtm <- as.integer(unlist(mean.TEMP[[1]] + 3.0 * sd.TEMP[[1]]))
          data_cont <- data_cont[, MinT := MinT ][MinT > TEMPExtm, MinT := as.integer(TEMPExtm)]
      } else if (varibl == 'MaxT') {
          mean.TEMP <- data_cont[ , lapply(.SD, mean, na.rm=TRUE), .SDcols = !'DATE']
          sd.TEMP <- data_cont[ , lapply(.SD, sd, na.rm=TRUE), .SDcols = !'DATE'] 
          TEMPExtm <- as.integer(unlist(mean.TEMP[[1]] + 3.0 * sd.TEMP[[1]]))
          data_cont <- data_cont[, MaxT := MaxT ][MaxT > TEMPExtm, MaxT := as.integer(TEMPExtm)]        
      } else if (varibl == 'MxRH') {
          data_cont <- data_cont[, MxRH := MxRH ][MxRH > 100, MxRH := as.integer(100)]
      } else if (varibl == 'MnRH') {
          data_cont <- data_cont[, MnRH := MnRH ][MnRH > 100, MnRH := as.integer(100)]         
      } else {
          print(varibl)
      }
      
      # Merge final data
      DT_cmb <- merge(DT_cmb, data_cont, by = 'DATE', all=TRUE)
    }
    rm(DT.out,seltvar,stationdata.DT.pyr)

    # Output file with ammendments for testing
    filedir <- file.path(paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/'))
    fileout <- paste0('IDW_AvgdCombdStatDat_',PYROMEIN,'.rds')
    ...
    DT_cmb[, PYROME := PYROMEIN]
    filesappend <- list(DT_cmb.mstr, DT_cmb) 
    DT_cmb.mstr <- rbindlist(filesappend, use.names=TRUE, fill=TRUE)  
    
    # Clean memory
    rm(DT_cmb,data_cont.RnDr,data_cont) 

  } # Select pyromes
} # All pyromes   

# Write combined file 
mastrfile <- paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/IDW_AvgdCombdStatDat_Allpyrms.rds')
saveRDS(DT_cmb.mstr, mastrfile)

ggplot() +
  # plot all simulations
  geom_line(data = DT_cmb[DATE > '2010-12-31'], aes(x = DATE, y = TEMP, color = 'red'), alpha = 1, size = 0.65) +
  labs(x="Date",y=expression('Precipitation Amount (inches)')) +
  theme(text = element_text(size=10))    

ggplot(stationdata.DT.pyr, aes(x = DATE, y = TEMP)) + 
  geom_line() + 
  facet_wrap(~ STATNUM, scales = 'free_y', ncol = 1)

test <- stationdata.DT.pyr[STATNUM == '044317']
ggplot() +
  # plot all simulations
  geom_line(data = test[DATE > '2016-12-31'], aes(x = DATE, y = TEMP, color = STATNUM, group=STATNUM), alpha = 1, size = 0.65) +
  geom_line(data = DT_cmb[DATE > '2016-12-31'], aes(x = DATE, y = TEMP, color = 'green'), alpha = 1, size = 0.65) +
  labs(x="Date",y=expression('Precipitation Amount (inches)')) +
  theme(text = element_text(size=10))
...
gridfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top4CloseGridPointsAllPyromesCWAtrbs.rds')
...
oldnames <- c('SIGName','Rain','RH')
newnames <- c('STATNUM','RAIN','ATMOIST')
...
cmbndgddatfile <- paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/AllPyromesOrigNARRGridDat_DDUPED.rds')
saveRDS(griddeddata.DT, cmbndgddatfile)

##################################################################################################################
## Section: Create combined time series of precipitation for each pyrome from NARR data
##################################################################################################################
...
for (ipyr in pyromes) { 
  ...
  if (!is.na(idx_one_in_two)) { 
    ...
    rm(data_cont.MxRH,data_cont.MnRH,DT_cmb,data_cont,cont_date_time,griddeddata.DT.pyr)    
...
for (ipyr in 1:length(pyromes)) { 
  ...
  if (!is.na(idx_one_in_two)) { 
    ...
    NARRFullFile <- file.path(paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/',NARRFilnm)) 
    ...
    StatFullFile <- file.path(paste0(ROOT_DATA_DIR,'/',OUTPUT_DIR,'/',StatFilnm))
...
# Calculate dew point at current RH and Temp (C) (http://bmcnoldy.rsmas.miami.edu/humidity_conversions.pdf)
RelHumdty <- function(TD,Temp){
  100*(exp((17.625*TD)/(243.04+TD))/exp((17.625*Temp)/(243.04+Temp)))
}

Temptre <- function(RH,TD){
  243.04*(((17.625*TD)/(243.04+TD))-log(RH/100))/(17.625+log(RH/100)-((17.625*TD)/(243.04+TD)))
}

DewPnt <- function(RH,Temp){
  243.04*(log(RH/100)+((17.625*Temp)/(243.04+Temp)))/(17.625-log(RH/100)-((17.625*Temp)/(243.04+Temp)))
}
celsius <- function(fahrenheit) {(5.0/9.0) * (fahrenheit-32)}
kelvin <- function(celsius) {celsius + 274.15}
fahrenheit<- function(celsius) {(celsius / (5.0/9.0)) + 32}  
