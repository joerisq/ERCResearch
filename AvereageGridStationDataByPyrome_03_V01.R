...
for (ipyr in pyromes) { 
  ...
  if (!is.na(idx_one_in_two)) { 
    ...
    mean.RAIN <- data_cont.RAIN[ , sapply(.SD, function(x) list(mean = mean(x[x!=0], na.rm=TRUE))), .SDcols = !'DATE'] 
    sd.RAIN <- data_cont.RAIN[ , sapply(.SD, function(x) list(sd = sd(x[x!=0], na.rm=TRUE))), .SDcols = !'DATE'] 
    #mean.RAIN <- data_cont.RAIN[ , lapply(.SD, mean, na.rm=TRUE), .SDcols = !'DATE']
    #sd.RAIN <- data_cont.RAIN[ , lapply(.SD, sd, na.rm=TRUE), .SDcols = !'DATE'] 
    RAINExtm <- as.numeric(unlist(mean.RAIN[[1]] + 3.0 * sd.RAIN[[1]]))
    RAINExtm <- 10.0
    data_cont.RAIN <- data_cont.RAIN[, RAIN := RAIN ][RAIN > RAINExtm, RAIN := as.numeric(RAINExtm)]
    # Extract RAIN *********************************************************************
