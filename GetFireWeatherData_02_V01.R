...
# Read Improved station list 
stationlistfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','FAM.NWCG.GOVFAM-files_StationList_',DATA_YR_OUT,'_CORCTD.rds')
stationlistfile.dt <- readRDS(stationlistfile)
...
#pyrssub <- c(17,56)
pyrssub <- seq(1,numpyr,1)

# Specify weather station files to copy
num.fw.sts <- 6 # Change to 11 for pyromes 17 and 56
...
for (i in 1:nrow(Pyrms_shp)) { 
  ...
  idx_one_in_two <- match(PYRRegincode, unlist(pyrssub)) 
  # Select subset of pyromes
  ...
  points(centroid.AOI,pch=2, col = 'Red')
  # Select the top 5
  stationlistfile.dt.rnkd <- stationlistfile.dt[distrank < num.fw.sts]
  ...
  } # Check pyrome is in select list
}
...
#stationlistfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top10CloseStationsAllPyromes_2019_P017.P056.rds')
...
for (i in 1:nrow(Pyrms_shp)) {
  ...
  PYRRegin <- Pyrms_shp[i,'PYROME']
  PYRRegincode <- unlist(Pyrms_shp@data[i, 'PYROME'])
  PyromesfOLDER <- paste0('Pyromes_2019','/','GriddedWeatherData','/',sprintf('%03d', PYRRegincode))
  output_dir_PYR <- file.path(ROOT_DATA_DIR, DATA_DIR, PyromesfOLDER)
...
# Write concatenated data.table of all pyromes and closest stations
gridfile.rds <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top4CloseGridPointsAllPyromes.rds')
...
stationlistfile <- paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/','Pyromes_2019/','Top5CloseStationsAllPyromes_2019.rds')
stationlistfile.dt <- readRDS(stationlistfile)
...
if (require(rgdal)) {
  rf <- writeRaster(landform, filename=paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/Elevation_GRID/landform.tif'), 
...
writeOGR(obj=WFgrds.spdf.trnsfrmd, dsn=paste0(ROOT_DATA_DIR,'/',DATA_DIR,'/Elevation_GRID'), 
